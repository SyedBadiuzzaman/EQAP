...
DATA_PATH = os.path.join(BASE_DIR, "data", "features", "EURUSD_1h_features.csv")
MODEL_SAVE_PATH = os.path.join(BASE_DIR, r"models\EUR_USD model", "xgb_1h_model.joblib")
...
def train_xgb_model(X_train: pd.DataFrame, y_train: pd.Series) -> XGBClassifier:
    ...
    logger.info("Training XGBoost model for 1H timeframe...")
    ...
    # Hyperparameters (maintained from 1m pattern or common defaults)
    model = XGBClassifier(
        n_estimators=300,
        max_depth=6,
        learning_rate=0.05,
...
def evaluate(model: XGBClassifier, X_test: pd.DataFrame, y_test: pd.Series, thresholds: List[float] = [0.35, 0.4, 0.45, 0.5]):
    ...
    logger.info("Evaluating 1H model performance...")
...
def save_model(model: XGBClassifier, path: str):
    ...
    logger.info(f"1H Model successfully saved to: {path}")
...
def main():
    try:
        ...
        logger.exception(f"An error occurred during the 1H training pipeline: {e}")
