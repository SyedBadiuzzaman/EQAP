...
class FeatureValidationConfig:
    ...
    min_non_null_ratio: float = 0.95  # Minimum 95% non-null values
    max_nan_threshold: float = 0.05  # Maximum 5% NaN allowed
    feature_bounds: Dict[str, Tuple[float, float]] = None  # (min, max) bounds
...
class FeatureEngineeringPipeline:
    ...
    def validate_input_data(self, df: pd.DataFrame) -> DataQualityMetrics:
        ...
        # Calculate metrics using numpy 2.2 compatible operations
        ...
        # Add warnings for high null percentages
    ...
    def preprocess_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Convert Date to datetime (numpy 2.2 compatible)
    ...
    def engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # --- Volume Features (numpy 2.2 compatible) ---
        # Using np.log1p for stability (works across numpy versions)
    ...
    def validate_features(self, df: pd.DataFrame) -> Dict[str, any]:
        ...
        # Check for infinite values (numpy 2.2 compatible)
        ...
        for col in numeric_cols:
            ...
            if inf_mask.any():
                ...
                # Replace infinite with NaN for later handling
    ...
    def create_target_and_clean(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Create classification target (1 if price up, 0 if down)
        ...
        # Drop raw Ichimoku features (keep transformed versions)
        ichimoku_raw = [
            "Ichimoku_conversion", "Ichimoku_base", "Ichimoku_spanA",
            "Ichimoku_spanB", "Ichimoku_lagging"
        ]
...
class FeatureTestStrategy:
    ...
    def test_feature_distributions(df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in numeric_cols:
            ...
            # Test 1: Check std dev (should be > 0 for useful features)
            std_dev = float(col_data.std())
            if std_dev == 0:
                test_results["issues"].append(f"Feature '{col}' has zero variance")
                test_results["passed"] = False
            
            # Test 2: Check skewness (extreme skew indicates data issues)
    ...
    def test_target_distribution(df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Warn if severely imbalanced (< 30% / > 70%)
...
def generate_ml_features(files: str) -> None:
    """
    Wrapper function for backward compatibility.
    Uses the new pipeline-based approach.
    """
    pipeline = FeatureEngineeringPipeline()
    try:
        df, validation_results = pipeline.run(files)
        
        # Run model testing strategy
...
def main():
    ...
    TARGET_FILES_cleaned = [
        "BTC_1d_cleaned.csv",
        "AAPL_1d_cleaned.csv",
        "EURUSD_1d_cleaned.csv",
        "XAUUSD_1d_cleaned.csv",
    ]
    
    logger.info("=" * 80)
    logger.info("EQUINOX FEATURE ENGINEERING PIPELINE (numpy 2.2 compatible)")
    logger.info("=" * 80)
